...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ size, extent, values: data.elevation, center: data.elevation[40] }, { headers: { "Cache-Control": "public, max-age=86400, s-maxage=604800" } });
  } catch { return NextResponse.json({ error: "elevation service unavailable" }, { status: 503 }); }
}
