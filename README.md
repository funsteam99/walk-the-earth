...
| 世界區塊串流 | 94% | 距邊界 70 公尺提前預載、快速銜接、15 秒逾時保護與六筆近期區塊快取；仍可強化多區塊同時並存 |
| 效能管理 | 94% | 合併、實例化、畫質分級、視距裁切、即時監測、自動降級、閒置降幀與背景分頁暫停；進階 LOD 可再強化 |
...
- 玩家靜止時降低 3D 繪製頻率；切換到其他分頁時暫停場景更新，避免影響影片播放與其他應用程式。
- 地圖與高程資料會在記憶體及瀏覽器保存六小時，最多保留六筆近期請求；重訪相同座標時可直接使用快取，降低頻寬與上游 API 負擔。
